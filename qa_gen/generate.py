...
def build_user_prompt(task, chunks_for_task):
    parts = [
        f"任务：{task.get('hint', '根据以下文档生成一条QA')}",
        f"答案类型倾向：{task.get('answer_type', 'single_fact')}（single_fact / multi_chunk / table_lookup）",
        f"问题分类：{task.get('category', '产品')}",
        f"意图：{task.get('intent', '查详情')} —— 请按此意图调整问题风格",
        "",
...
def gen_one_qa(task, by_id, all_chunks, sys_prompt=None):
    chunks_for_task = [by_id[cid] for cid in task.get("chunk_ids", []) if cid in by_id]
    has_question_override = bool(task.get("question_override"))

    # 带 question_override 的负样本：直接返回，不调 LLM
    if has_question_override:
        return {
            "category": task.get("category", "产品"),
            "question": task["question_override"],
            "answer": "該文檔未提供該信息，無法從本文檔回答。",
            "document": task.get("product_name", ""),
            "page": "",
            "supporting_text": "",
            "gold_chunk_ids": "",
            ...
            "intent": "拒答",
            "primary_chunk_id": "",
            "section_title": "[NEGATIVE_AUTO]",
            "task_key": task["key"],
        }

    if not chunks_for_task:
        log_event(f"[SKIP] 任务 {task.get('key', '?')} 找不到 chunk_ids，跳过。")
        return None
    user_prompt = build_user_prompt(task, chunks_for_task)
    last_err = None
    # 使用传入的 sys_prompt 或默认 SYS_PROMPT
    if sys_prompt is None:
        from qa_gen.buckets import SYS_PROMPT
    ...
    for attempt in range(MAX_RETRIES_PER_ITEM + 1):
        ...
        # 文档名：自动模式从 chunk 的 source 字段提取文件名
        doc_name = DOC_NAME
        if primary_id in by_id:
            source = by_id[primary_id].get("source") or ""
            if source:
                doc_name = source.split("/")[-1] if "/" in source else source
                doc_name = doc_name.replace(".pdf", "") if doc_name.endswith(".pdf") else doc_name
