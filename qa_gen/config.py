...
# -------- 配置 --------
# 手动模式的默认输入文件（仅用于 python generate_qa.py 无 --auto 时）
INPUT_JSONL = "data/安达/Chubb_常見疾病指引_paged.jsonl"
OUT_DIR     = "./output"
OUT_XLSX    = os.path.join(OUT_DIR, "QA测评集.xlsx")
OUT_LOG     = os.path.join(OUT_DIR, "run_log.md")
DOC_NAME    = "Chubb_常見疾病指引.pdf"
...
def get_client():
    """延迟初始化 OpenAI client，避免导入时缺少 API key 报错。"""
    global client
    if client is None:
        client = OpenAI(
